...
## 🎉 **סבב 9: Phase 2 Behavior Engine** (6 בינואר 2026)

### ✅ Behavior Detection Engine - מנוע ניתוח התנהגות

#### 1. 5 אלגוריתמי זיהוי
- **Recurring Detection** - מנויים וחיובים חוזרים (variance < 15%, interval ~30 ימים)
- **Spike Detection** - קפיצות (> 150% מהממוצע)
- **Trend Detection** - מגמות עולות/יורדות (> ±15%)
- **Day Pattern** - דפוסי יום בשבוע
- **Seasonality** - עונתיות (outliers > 1.5 std)

#### 2. קובץ חדש: `lib/analysis/behavior-engine.ts`
- `runFullAnalysis()` - מנתח 3 חודשים אחרונים
- `detectRecurring()` - מזהה מנויים
- `detectSpikes()` - מזהה קפיצות
- `detectTrends()` - מזהה מגמות
- `detectDayPatterns()` - מזהה דפוסי יום
- `detectSeasonality()` - מזהה עונתיות
- `generateVendorSummaries()` - סיכום לפי vendor

#### 3. Phase Handlers ב-phi-router.ts
- `handleBehaviorPhase()` - routing לפקודות
- `startBehaviorAnalysis()` - מפעיל ניתוח
- `showBehaviorSummary()` - מציג תוצאות
- `sendBehaviorSummary()` - WhatsApp עם 5 הודעות
- `transitionToGoals()` - מעבר לשלב הבא

#### 4. API Endpoint: `/api/behavior`
- GET - קבלת ניתוח אחרון
- POST - הפעלת ניתוח חדש (עם periodMonths)

#### 5. שינויים ב-Flow
- סיום classification → עובר ל-behavior (לא monitoring)
- behavior → goals (כפתור "המשך")

#### 6. הודעות WhatsApp
```
📊 *ניתוח התנהגות - 3 חודשים*
📈 סיכום: סה"כ/ממוצע/קבועות/משתנות
...
### 📊 סטטיסטיקות:
- **1 קובץ חדש:** `lib/analysis/behavior-engine.ts`
- **1 API route:** `/api/behavior`
- **6 פונקציות:** detection + analysis
- **5 handlers:** behavior phase
