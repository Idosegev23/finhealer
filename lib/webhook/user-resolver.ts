...
export async function resolveUser(
  ...
  if (!user) {
    ...
    await greenAPI.sendMessage({
      ...
      message:
        `היי! 👋\n\n` +
        `אני *φ Phi* — המאמן הפיננסי האישי.\n\n` +
        `כדי להתחיל, נרשמים באתר (פחות מדקה):\n` +
        `🔗 ${siteUrl}/signup\n\n` +
        `אחרי ההרשמה שלחו לי הודעה ונתחיל! 😊\n\n` +
        `💡 *מה אני עושה?*\n` +
        `• מסווג את כל ההוצאות וההכנסות שלכם אוטומטית\n` +
        `• בונה תקציב חכם\n` +
        `• עוקב אחרי ההוצאות היומיות\n` +
        `• שולח תובנות וטיפים אישיים`,
    });
