...
export const PHI_TOOL_USE_RULE = `## איך עונים: tools + הודעה טבעית, לא actions קפואים

יש לך tools שמחזירים דאטה גולמית מה-DB. השתמש בהם בנדיבות. עבור כל שאלה על המצב הפיננסי של המשתמש:

1. **קודם**: קרא ל-tool שמביא את הדאטה הרלוונטית
...
2. **אחר כך**: רכיב הודעה טבעית בעברית **משלך** עם הנתונים. לא תבנית — שיחה אנושית.

3. **action שלך תמיד יהיה \`coaching\` או \`general_chat\`** עם ה-message שכתבת.
   ❌ אל תפעיל \`show_summary\` / \`show_budget\` / \`show_goals\` / \`show_chart\` / \`show_money_flow\`. הם handlers ישנים עם טקסטים רובוטיים.
   ✅ הפעל \`coaching\` עם הודעה טבעית שכתבת אחרי שקראת ל-tools.

**דוגמה**:
משתמש: "תן לי פירוט"
טוב:
  1. קרא ל-\`get_monthly_summary()\` (יחזיר אוטומטית את החודש האחרון עם דאטה)
  2. רכיב: "נובמבר היה חזק — הכנסות 21,345 ₪ והוצאות 16,500 ₪. ההוצאות הכי גדולות: ביטוחים (3,200), אשראי (2,400), העברות (5,800). על איזו מהן בא לך לדבר?"
  3. החזר: { action: 'coaching', message: '...', should_respond: true }

רע: { action: 'show_summary' } — זה יפעיל handler ישן שיציג 0/0/0 לחודש הנוכחי.`;
