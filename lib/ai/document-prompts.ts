...
🚨 **קריטי - חובה לחלץ את כל התנועות מכל החודשים!** 🚨
**הדוח עשוי לכלול 2-3 חודשים של עסקאות!** חלץ את כל העסקאות מכל החודשים, לא רק מהחודש הראשון!
...
**זה לא דוגמה - זה הדוח המלא! כל העמודים, כל החודשים!**
...
🚨 **קריטי - חובה לחלץ את כל התנועות מכל החודשים!** 🚨
**הדוח עשוי לכלול 2-3 חודשים של תנועות!** חלץ את כל התנועות מכל החודשים, לא רק מהחודש הראשון!
...
## 🚨🚨🚨 **בנק לאומי - שיטת הזיהוי העיקרית!** 🚨🚨🚨

**בבנק לאומי יש שתי עמודות נפרדות לסכומים:**
- **עמודה "זכות"** (בדרך כלל משמאל) = הכנסות → type: "income" ✅
- **עמודה "חובה"** (בדרך כלל מימין) = הוצאות → type: "expense" ❌

**🔥 שים לב! בבנק לאומי כל הסכומים חיוביים - ההבדל הוא באיזו עמודה הם נמצאים! 🔥**

**דוגמה מדוח בנק לאומי:**
| תאריך | תיאור | זכות | חובה | יתרה |
|-------|--------|------|------|------|
| 05/11 | משכורת | 12,000.00 | | 15,000.00 |
| 10/11 | ויזה | | 2,500.00 | 12,500.00 |
| 15/11 | ארנונה | | 450.00 | 12,050.00 |

**תוצאה נכונה:**
- משכורת 12,000 → **income** (כי הסכום בעמודת זכות!)
- ויזה 2,500 → **expense** (כי הסכום בעמודת חובה!)
- ארנונה 450 → **expense** (כי הסכום בעמודת חובה!)

## **שיטות זיהוי נוספות (לבנקים אחרים):**

**מצב 2: עמודה אחת עם סימן**
- **מספר חיובי (+)** או בלי סימן בעמודת זכות = הכנסה
- **מספר שלילי (-)** או בעמודת חובה = הוצאה

**מצב 3: רק עמודה אחת - הסתכל על היתרה**
- אם היתרה **עלתה** → income (הכנסה)
- אם היתרה **ירדה** → expense (הוצאה)

**דוגמאות עם סימנים:**
- "-500.00" או "500.00-" → expense (הוצאה)
- "2500.00" בעמודת זכות → income (הכנסה)
- "500" בעמודת חובה → expense (הוצאה)
...
### **🚨🚨🚨 חריגים קריטיים - אלה לא הכנסות אמיתיות! 🚨🚨🚨**

**❌ אל תסווג כהכנסה:**

1. **"החזרת חיוב" / "החזר חיוב" / "ביטול חיוב"**
   - זיהוי: "החזרת חיוב", "החזר חיוב", "זיכוי מחזיר", "ביטול", "cancellation", "reversal"
   - **זה לא הכנסה חדשה - זה ביטול של הוצאה שכבר הייתה!**
   - **פתרון:** אל תכלול בתנועות בכלל! דלג על תנועות אלו.
   - דוגמאות: 
     - "החזרת חיוב כ.א.ל מסיבת אין הרשאה" → ❌ לא להכליל
     - "זיכוי החזר חיוב הפניקס" → ❌ לא להכליל
     - "ביטול חיוב מסיבת אכ\"מ" → ❌ לא להכליל

2. **"זיכוי הנחות" / "זיכוי מפתח" / "זיכוי קאשבק"**
   - זיהוי: "זיכוי הנחות", "מפתח", "cashback", "הנחה", "קאשבק"
   - **זה הנחה על הוצאה קיימת, לא הכנסה חדשה!**
   - **פתרון:** אל תכלול בתנועות בכלל! דלג על תנועות אלו.
   - דוגמאות:
     - "זיכוי הנחות מפתח מכרטיס 3943" → ❌ לא להכליל
     - "קאשבק ויזא" → ❌ לא להכליל

3. **תיקוני טעויות בנק**
   - זיהוי: "תיקון טעות", "טעות בנקאית", "correction", "bank error"
   - **זה לא הכנסה - זה תיקון של טעות!**
   - **פתרון:** אל תכלול בתנועות בכלל! דלג על תנועות אלו.

**✅ רק אלה הכנסות אמיתיות:**
- משכורת, עצמאי, עסק
- קצבאות ממדינה
- החזר מס (מס הכנסה)
- גמלאות, ביטוח לאומי
- השקעות (דיבידנד, ריבית)
- השכרה
- מתנה, ירושה
- העברות מאנשים אחרים
...
## **🎯 זיהוי מסמכים חסרים (missing_documents) - חשוב מאוד!**

**בנוסף לחילוץ התנועות, זהה כל מסמך שהמשתמש צריך להעלות כדי לקבל תמונה מלאה!**
**הכל קשור אחד בשני - משכורת → תלוש → פנסיה → ביטוחים**

### **1. כרטיסי אשראי (credit):**
- **זיהוי:** "ויזא", "מאסטרקארד", "אמקס", "דינרס", "ישראכרט", "כאל", "מקס", "לאומי קארד", "חיוב כרטיס"
- **חלץ:** card_last_4, charge_date, charge_amount, period_start, period_end
- **דוגמה:** "חיוב ויזא ****1234 - 5,890 ₪" (10/11/2024) → צריך דוח אשראי 10-11/2024

### **2. משכורות / תלושי שכר (payslip):**
- **זיהוי:** "משכורת", "שכר", "salary", שם מעסיק ידוע, "העברה שכר"
- **חלץ:** salary_date, salary_amount, employer, month
- **חשוב:** מתלוש השכר נקבל גם: הפקדות פנסיה, קרן השתלמות, ביטוח מנהלים!
- **דוגמה:** "משכורת חברת ABC - 15,234 ₪" → צריך תלוש שכר לחודש המתאים

### **3. משכנתא (mortgage):**
- **זיהוי:** "משכנתא", "הלוואת דיור", "mortgage", "בנק X משכנתא"
- **חלץ:** payment_date, payment_amount, lender
- **דוגמה:** "תשלום משכנתא בנק דיסקונט - 4,567 ₪" → צריך דוח משכנתא עדכני

### **4. הלוואות (loan):**
- **זיהוי:** "הלוואה", "פירעון הלוואה", "החזר הלוואה", "loan", "הלוואת X"
- **חלץ:** payment_date, payment_amount, lender, loan_number (אם יש)
- **דוגמה:** "פירעון הלוואה 123456 - 1,500 ₪" → צריך דוח הלוואות מהבנק

### **5. ביטוחים (insurance):**
- **זיהוי:** "ביטוח חיים", "ביטוח בריאות", "ביטוח רכב", "ביטוח דירה", "ביטוח סיעודי",
  "הראל", "מגדל", "כלל", "הפניקס", "מנורה", "איילון", "שלמה", "AIG", "פוליסה"
- **חלץ:** payment_date, payment_amount, provider, insurance_type
- **סוגי ביטוח:** life, health, car, home, disability, critical_illness
- **דוגמה:** "הראל ביטוח - 450 ₪" → צריך פוליסת ביטוח מהראל

### **6. פנסיה / גמל / השתלמות (pension):**
- **זיהוי:** "קרן פנסיה", "קופת גמל", "קרן השתלמות", "ביטוח מנהלים",
  "מיטב דש", "אלטשולר שחם", "מור", "הלמן אלדובי", "מגדל", "הראל", "כלל", "פניקס"
- **חלץ:** payment_date, payment_amount, provider, fund_type
- **סוגי קרן:** pension_fund, provident_fund, advanced_study_fund, managers_insurance
- **דוגמה:** "מיטב דש פנסיה - 800 ₪" → צריך דוח פנסיה שנתי

### **7. חיסכון והשקעות (savings/investment):**
- **זיהוי:** "פיקדון", "חיסכון", "קופת גמל להשקעה", "השקעות", "תיק השקעות"
- **חלץ:** transfer_date, transfer_amount, destination
- **דוגמה:** "העברה לפיקדון - 2,000 ₪" → צריך דוח פיקדונות/השקעות

### **🔗 קשרים בין מסמכים - חשוב להבין!**

| תנועה בדוח בנק | מסמך נדרש | מה נקבל ממנו |
|----------------|-----------|--------------|
| משכורת | תלוש שכר | ברוטו, ניכויים, פנסיה, קה"ש |
| חיוב אשראי | דוח אשראי | פירוט הוצאות |
| משכנתא | דוח משכנתא | קרן, ריבית, יתרה |
| הלוואה | דוח הלוואות | פירוט כל ההלוואות |
| ביטוח | פוליסה | סוג כיסוי, תנאים |
...

{
  ...
  "missing_documents": [
    ...
    },
    {
      ...
      "employer": "חברת XYZ",
      "month": "2024-08",
      "description": "משכורת חברת XYZ - צריך תלוש לראות פנסיה וניכויים",
      "priority": "high",
      "related_documents": ["pension", "advanced_study_fund"]
    },
    {
      "type": "mortgage",
