...
## **🔥 3. מידע חיוב (billing_info) - קריטי ביותר! חובה למלא!**
- next_billing_date (מועד החיוב הבא בבנק - YYYY-MM-DD)
- next_billing_amount (הסכום הכולל שיחוייב בבנק - ללא עיגול!)
- card_last_digits (4 ספרות אחרונות של הכרטיס)
- card_company (שם חברת האשראי: "מקס" / "כאל" / "ישראכרט" / "לאומי קארד" / "אמריקן אקספרס")

**🚨 קריטי - billing_info חייב להיות מלא! 🚨**
- זה המידע שמאפשר לנו להתאים בין דוח האשראי לחיוב בדוח הבנק
- בלי billing_info, נוצרת ספירה כפולה של הוצאות!
- חפש בראש הדוח: "מועד חיוב", "תאריך חיוב", "יחוייב בתאריך"
- חפש: "סך לחיוב", "סכום לתשלום", "סה״כ לחיוב", "TOTAL"
- חפש: "כרטיס מספר", "4 ספרות אחרונות"
- **אם לא מצאת next_billing_amount** — חשב אותו: סכום כל העסקאות בדוח
- **אם לא מצאת next_billing_date** — השתמש בתאריך הסמוך לתחילת החודש הבא (בדרך כלל ה-2 או ה-3 בחודש)
...
🚨 **קריטי**: דוח מסלקה הוא בדרך כלל **30-40 עמודים**.
המידע על התוכניות מתחיל בעמוד "סטטוס פוליסות" (עמוד 2 בערך) ונמשך **בכל עמודי "רשימת תוכניות" וכל "יתרות לפי תקופה"**.
חובה לסרוק את **כל העמודים** — לא רק העמודים הראשונים. **אסור להחסיר אף תוכנית**.

🎯 **דרך עבודה מומלצת**:
1. קרא את עמוד "סטטוס פוליסות" — שם יש טבלה עם **כל** התוכניות (מספר רץ).
2. עבור על כל שורה בטבלה → צור רשומה ב-pension_plans או insurance_policies.
3. השתמש בעמודי "רשימת תוכניות" המפורטים כדי להוסיף סכומים, מסלולי השקעה ודמי ניהול.

🚨 אם בעמוד "סטטוס פוליסות" יש 12 תוכניות — **חייבות להופיע 12** בפלט הסופי. ספור!
...
- **תשואה (annual_return)** — אחוז תשואה שנתית (ממוצע 5 שנים אם יש, אחרת 12 חודשים אחרונים)
- **תשואות היסטוריות (returns_history)**: { last_12m, last_24m, last_36m, last_60m, std_36m, std_60m } — מהעמוד "פירוט מסלולי השקעה"

🚨 **חשוב — תשואות**: בכל דוח מסלקה יש סקציה בשם **"פירוט מסלולי השקעה"** או **"מסלולי השקעה והרכב נכסים"** (מיקום העמוד משתנה בין דוחות וגופים — אל תחפש לפי מספר עמוד). הסקציה מכילה טבלה עם **תשואות שנתיות ממוצעות** ב-12/24/36/60 חודשים אחרונים, סטיות תקן ב-36/60 חודשים, וחשיפה למניות. **חובה לחלץ את הנתונים האלה** עבור כל מסלול ולשייך לתוכנית המתאימה לפי שם המסלול (\`investment_track\`). חיפוש בכל הדוח עד שתמצא את הסקציה.

לדוגמה אם בעמוד "פירוט מסלולי השקעה" יש שורה:
"מנורה מבטחים פנסיה עוקב מדד S&P500 — 12 חודשים: 7.98%, 24 חודשים: 14.21%, 36 חודשים: 13.82%, 60 חודשים: %, סטיית תקן 36ח: 2.15%, חשיפה למניות: 73.17%"

צריך להחזיר עבור התוכנית המתאימה:
  "annual_return": 14.21,
  "returns_history": { "last_12m": 7.98, "last_24m": 14.21, "last_36m": 13.82, "last_60m": null, "std_36m": 2.15, "stock_exposure": 73.17 }
