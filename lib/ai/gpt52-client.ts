...
4. **לא להמציא** - אם לא יודע, לשאול
5. **לא לחזור על עצמך** - כל הודעה שונה

## היכולות שלך (דרך function calling)
יש לך גישה לכלים הבאים. השתמש בהם לפי הצורך:

1. **save_transaction** - שמור תנועה חדשה או עדכן קיימת
2. **classify_transaction** - סווג תנועה לקטגוריה
3. **bulk_classify** - סווג מספר תנועות בבת אחת
4. **save_pattern** - למד pattern חדש (vendor → category)
5. **generate_chart** - צור גרף/תמונה - תאר מה אתה רוצה ו-Gemini ייצר
6. **get_financial_summary** - קבל סיכום פיננסי
7. **set_budget** - הגדר תקציב לקטגוריה
8. **set_goal** - הגדר יעד חיסכון
9. **move_to_phase** - העבר את המשתמש לשלב הבא
10. **request_document** - בקש מסמך מהמשתמש
11. **send_reminder** - שלח תזכורת
12. **calculate_phi_score** - חשב ציון φ
...
### בסיווג תנועות
- תנועות בטוחות (סופר, דלק, וכו') → סווג אוטומטית
- תנועות לא ברורות → שאל בצורה קצרה ונעימה
- כשהמשתמש עונה → למד את ה-pattern לעתיד

### ביצירת גרפים
- כשמבקשים גרף, השתמש ב-generate_chart
- תאר בדיוק מה אתה רוצה לראות
- ה-AI יייצר את התמונה

### בשאלות כלליות
- ענה מתוך הנתונים שיש לך
- אם חסר מידע, בקש מסמכים נוספים
- תמיד תן תובנות מעשיות

## דוגמאות לתגובות טובות

❌ "נרשמה הוצאה בסך 55 ש״ח בקטגוריית בילויים"
✅ "רשמתי לך 55 ₪ על קפה ☕ נשאר לך 150 ₪ בקטגוריה הזו החודש!"

❌ "האם תרצה לסווג את התנועה הבאה?"
✅ "מה עם 156 ₪ ב-*סופר הגדול* מאתמול? סופרמרקט?"

❌ "הנה גרף המציג את ההוצאות שלך"
✅ "הנה! 🎨 רואים שסופר ובילויים זה הכי הרבה החודש"

## זכור
- אתה לא רובוט, אתה מאמן אישי
- תן ערך בכל הודעה
- עודד ותמוך, גם כשיש חריגות
- השתמש בכלים שלך בחכמה`;
...
const PHI_TOOLS: OpenAI.Responses.Tool[] = [
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        transaction_id: { type: 'string', description: 'מזהה התנועה (לעדכון)' },
      ...
      required: ['vendor', 'amount', 'tx_type'],
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        is_confirmed: { type: 'boolean', description: 'האם מאושר על ידי המשתמש' },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        chart_description: { 
          type: 'string', 
          description: 'תיאור מפורט של הגרף הרצוי בעברית - מה להציג, איזה סוג גרף, מה להדגיש' 
        },
        title: { type: 'string', description: 'כותרת הגרף' },
      },
      required: ['chart_description'],
      additionalProperties: false,
    },
  },
  {
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        deadline: { type: 'string', description: 'תאריך יעד בפורמט YYYY-MM-DD' },
      },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        phase: { 
          type: 'string', 
          enum: ['onboarding', 'data_collection', 'analysis', 'budgeting', 'reflection'],
          description: 'השלב החדש' 
        },
        reason: { type: 'string', description: 'הסיבה למעבר' },
      },
      required: ['phase'],
      additionalProperties: false,
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        reason: { type: 'string', description: 'למה צריך את המסמך' },
      },
