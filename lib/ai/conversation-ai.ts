...
ההודעה צריכה להיות:
1. מזמינה וחמה - לא פורמלית
2. מסבירה בקצרה מה הולכים לעשות ביחד
3. יוצרת תחושה של "וואו, זה משהו אחר"
4. מסתיימת בשאלה על השם

📝 מה להסביר (בקצרה וברור):
- מי אתה ומה השם שלך אומר
- מה התהליך: נכיר → תשלח דוחות → אני מנתח → ביחד נבנה תמונה ברורה
- למה זה שונה: לא שיפוטי, בקצב שלך, אני לומד אותך
- מה היעד: שליטה ובהירות על הכסף

❌ מה לא לעשות:
- לא יותר מדי אימוג'ים (1-2 מקסימום)
- לא ארוך מדי (6-10 שורות)
- לא רשימות ארוכות
- לא להיות גנרי או משעמם

✅ דגשים:
- השתמש ב-*בולד* לדברים חשובים
- הסימן φ חייב להופיע ליד השם
- סיום עם שאלה על השם
...
החזר רק את ההודעה.`;
  ...
  try {
    const response = await chatWithGPT5Fast(
      'צור הודעת פתיחה ראשונה למשתמש חדש',
      systemPrompt,
